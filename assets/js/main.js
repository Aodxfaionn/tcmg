...
const showModal = function () {
  let btns = document.querySelectorAll(".js-modal");
  let modals = document.querySelectorAll(".modal");
  let modalName;

  btns.forEach((item) => item.addEventListener("click", selectModal));

  function selectModal() {
    modals.forEach((item) => item.classList.remove("open"));
    modalName = this.dataset.modalname;
    modals.forEach((item) => {
      if (item.classList.contains(modalName)) openModal(item);
    });
  }
};
...
setTimeout(startFirstAnimation, 1200);

gsap.to(".one-svg.two__circle-one", {
  scrollTrigger: {
    trigger: ".one",
    // markers: true,
    start: "top",
...
gsap.to(".one-svg.two__circle-two", {
  scrollTrigger: {
    trigger: ".one",
    // markers: true,
    start: "500px",
